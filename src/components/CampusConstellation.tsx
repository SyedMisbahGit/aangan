...
export const CampusConstellation = () => {
  ...
  const getMoodColor = (mood: string) => {
    switch (mood) {
      ...
      case "warm": return "text-pink-400";
  ...
  const getGlowIntensity = (activity: string) => {
    switch (activity) {
      case "high": return "shadow-lg shadow-purple-500/50 animate-pulse";
      case "medium": return "shadow-md shadow-purple-500/30";
      default: return "shadow-sm shadow-purple-500/10";
    }
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setZones(prev => prev.map(zone => {
        if (Math.random() > 0.8) {
          const activities: ("high" | "medium" | "low")[] = ["high", "medium", "low"];
          const moods: ("hot" | "cool" | "warm" | "neutral")[] = ["hot", "cool", "warm", "neutral"];
        ...
        <div className="relative bg-black/20 rounded-lg h-64 overflow-hidden">
          ...
          {zones.map((zone) => {
            ...
            return (
              ...
              >
                {/* Zone Node */}
                <div className={`relative ${getActivitySize(zone.activity)} ${getGlowIntensity(zone.activity)} rounded-full bg-purple-500/30 border border-purple-400/50 flex items-center justify-center hover:scale-110 transition-all duration-300`}>
                  <span className="text-xs">{zone.emoji}</span>
                  
                  {/* Mood Indicator */}
                  <div className="absolute -top-1 -right-1">
                    <MoodIcon className={`h-3 w-3 ${getMoodColor(zone.mood)} animate-pulse`} />
                  </div>
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                  <div className="bg-black/80 backdrop-blur-md rounded-lg p-2 text-xs text-white min-w-max">
                    ...
                    <div className="text-gray-300">{zone.whisperCount} whispers</div>
                    <div className="text-gray-400 capitalize">{zone.mood} • {zone.activity} activity</div>
                  </div>
                </div>
              </div>
            );
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {zones.slice(0, 4).map((zone) => (
              <div key={zone.id} className="flex items-center justify-between p-2 bg-white/5 rounded">
                <span className="text-gray-300">{zone.emoji} {zone.name}</span>
