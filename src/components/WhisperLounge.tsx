...
export const WhisperLounge = () => {
  ...
  const samplePosts: Omit<AmbientPost, 'isVisible' | 'opacity'>[] = [
    {
      id: "1",
      content: "Late night thoughts hit different when you're alone in the library. Sometimes silence speaks louder than words.",
    ...
    {
      id: "2", 
      content: "Found a handwritten note tucked in an old textbook today. Made me wonder about all the stories these walls have witnessed.",
      category: "discovery",
      mood: "curious",
      timestamp: new Date(),
    },
    {
      id: "3",
      content: "The way morning light filters through the hostel corridor... there's poetry in the ordinary moments we usually miss.",
    ...
    {
      id: "4",
      content: "Sometimes I write messages I'll never send, words that exist only in the space between thought and courage.",
      category: "confession",
      mood: "vulnerable",
      timestamp: new Date(),
    },
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentPost) {
        setCurrentPost(prev => prev ? { ...prev, opacity: Math.max(0, prev.opacity - 0.1) } : null);
  ...
  const handleInteraction = (type: 'seen' | 'echo' | 'fade') => {
    ...
    switch (type) {
      case 'seen':
        ...
        setCurrentPost(prev => prev ? { ...prev, mood: 'acknowledged' } : null);
        break;
      case 'echo':
    ...
    return (
      <Card className="bg-gradient-to-br from-slate-900/20 to-purple-900/20 backdrop-blur-lg border-white/10 p-12 text-center min-h-[60vh] flex items-center justify-center">
        <div className="space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-light text-white mb-4">Whisper Lounge</h2>
            <p className="text-gray-300 text-sm mb-8 max-w-md mx-auto leading-relaxed">
              A gentle space where thoughts drift by like clouds. No pressure to engage, just presence and peace.
            </p>
        ...
        {currentPost ? (
          ...
          >
            ...
            <div className="flex justify-center">
              <Badge className={`${getMoodBg(currentPost.mood)} ${getMoodColor(currentPost.mood)} text-xs px-4 py-2 rounded-full animate-pulse`}>
                {currentPost.mood}
              </Badge>
            </div>

            {/* Content */}
            <div className={`${getMoodBg(currentPost.mood)} rounded-2xl p-8 backdrop-blur-md border border-white/5`}>
