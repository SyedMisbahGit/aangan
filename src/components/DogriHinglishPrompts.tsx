...
const DogriHinglishPrompts: React.FC = () => {
  ...
  const categories = [
    { id: 'all', name: 'All Prompts', icon: <MessageCircle className="w-4 h-4" /> },
    { id: 'poetic', name: 'Poetic', icon: <Heart className="w-4 h-4" /> },
    { id: 'humorous', name: 'Humorous', icon: <Sparkles className="w-4 h-4" /> },
    ...
    <div className="whisper-orb floating-orb p-8 max-w-6xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="kinetic-text-slow text-gray-300 max-w-2xl mx-auto">
          Expanded CUJ prompt libraries with poetic, humorous, and senior-style tones. 
          Cultural context meets emotional expression.
        </p>
      </div>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className={`
                  ...
                  ${prompt.category === 'poetic' ? 'bg-pink-500/20 text-pink-300' :
                    prompt.category === 'humorous' ? 'bg-yellow-500/20 text-yellow-300' :
                    prompt.category === 'senior' ? 'bg-blue-500/20 text-blue-300' :
                    prompt.category === 'cultural' ? 'bg-green-500/20 text-green-300' :
              ...
              <div className="text-right">
                <div className="text-sm font-bold text-white">{prompt.usage}</div>
      ...
      <div className="mt-8 whisper-glass p-6 rounded-2xl">
        <h3 className="text-lg font-semibold text-white mb-4">Prompt Usage Statistics</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
          ...
          <div>
            <div className="text-2xl font-bold text-white">
              {Math.round(prompts.reduce((sum, p) => sum + p.usage, 0) / prompts.length)}
            </div>
