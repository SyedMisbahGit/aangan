...
export const TimeCapsules = () => {
  ...
  const [deliveryType, setDeliveryType] = useState<"self" | "anonymous">("self");
  const [deliveryTime, setDeliveryTime] = useState("");
  const [pendingDelivery, setPendingDelivery] = useState<TimeCapsule | null>(null);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setCapsules(parsed.map((capsule: any) => ({
        ...capsule,
  ...
  const checkDeliveries = () => {
    ...
    const deliverable = parsed.find((capsule: any) => 
    ...
    return (
      <Card className="bg-gradient-to-br from-amber-900/20 to-orange-900/20 backdrop-blur-lg border-amber-400/20 p-8 text-center">
        <div className="space-y-6">
          ...
          <div>
            <h3 className="text-2xl font-light text-white mb-2">A letter arrived</h3>
            <p className="text-amber-200 text-sm mb-2">
              From {pendingDelivery.type === 'self' ? 'yourself' : 'a kindred spirit'}
            </p>
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-br from-blue-900/20 to-indigo-900/20 backdrop-blur-lg border-white/10 p-8">
        <div className="space-y-6">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-light text-white">Send to the Future</h2>
          </div>

          <p className="text-gray-300 text-sm">
            Write a letter to your future self or let someone unknown find comfort in your words.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Select value={deliveryType} onValueChange={(value: "self" | "anonymous") => setDeliveryType(value)}>
              ...
              <SelectContent className="bg-slate-800/90 border-white/20 backdrop-blur-lg">
                <SelectItem value="self" className="text-white focus:bg-white/10">
                  My future self
                </SelectItem>
                <SelectItem value="anonymous" className="text-white focus:bg-white/10">
      ...
      {capsules.filter(c => !c.isDelivered).length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-light text-white">Traveling Through Time</h3>
          {capsules.filter(c => !c.isDelivered).map((capsule) => (
            <Card key={capsule.id} className="bg-white/5 backdrop-blur-lg border-white/10 p-6">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-400 text-xs">
                      to {capsule.type === 'self' ? 'future you' : 'someone special'}
                    </span>
                  </div>
