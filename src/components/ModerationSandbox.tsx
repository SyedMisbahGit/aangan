...
import { TestTube, Send, AlertTriangle, CheckCircle, XCircle, Brain } from "lucide-react";
...
export const ModerationSandbox = () => {
  ...
  const analyzeCulturalContent = (text: string) => {
    ...
    const distressSignals = /\b(thak gaya|kar baithunga|mar jaana|khatam kar|zinda nahi)\b/gi;
    if (distressSignals.test(text)) {
      flags.push("Mental health concern detected");
      action = "flagged";
      confidence = 0.92;
      culturalContext = "Hindi expression of distress - requires mental health protocol";
    }
    ...
    if (slangWords.test(text)) {
      if (text.includes("chhapri")) {
        culturalContext = "'Chhapri' - colloquial term, context-dependent offensiveness";
        action = "flagged";
    ...
    const academicStress = /\b(attendance|placement|exam|marks|tension|stress|prof)\b/gi;
    if (academicStress.test(text) && !flags.length) {
      culturalContext = "Academic stress expression - common during exam/placement season";
    }

    // Positive hinglish expressions
    const positiveHinglish = /\b(mast|bindaas|dhinchak|solid|kamaal|badhiya)\b/gi;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-white">Moderation Laboratory</h2>
          <p className="text-cyan-300 text-sm">Test cultural AI understanding • Hindi-English detection</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {testResults.map((test) => (
          <Card key={test.id} className="bg-white/5 backdrop-blur-lg border-white/10 p-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Badge className={getActionColor(test.result.action)}>
                  {getActionIcon(test.result.action)}
                  <span className="ml-1">{test.result.action.toUpperCase()}</span>
                </Badge>
              ...
              {test.result.flags.length > 0 && (
                <div className="space-y-2">
                  <p className="text-white text-sm font-medium">Detected Issues:</p>
              ...
              {test.result.culturalContext && (
                <div className="p-3 bg-purple-500/10 rounded-lg border border-purple-500/20">
                  <p className="text-purple-200 text-sm">
                    <strong>Cultural Context:</strong> {test.result.culturalContext}
                  </p>
                </div>
              )}
            </div>
