...
export const LockedWhispers = () => {
  ...
  const [selectedAction, setSelectedAction] = useState<"lock-forever" | "burn-after-reading" | null>(null);
  ...
  const handleLockWhisper = () => {
    ...
    localStorage.setItem('whisper-locked-entries', JSON.stringify(updatedWhispers));
    
    // Unlock themes based on action
    const newTheme = selectedAction === "lock-forever" ? "Sacred Vault" : "Phoenix Ashes";
    if (!unlockedThemes.includes(newTheme)) {
      const updatedThemes = [...unlockedThemes, newTheme];
      setUnlockedThemes(updatedThemes);
      localStorage.setItem('whisper-unlocked-themes', JSON.stringify(updatedThemes));
    }
      ...
      <Card className="bg-white/5 backdrop-blur-lg border-white/10 p-6">
        <div className="space-y-6 text-center">
          ...
          <div className="space-y-4">
            <h3 className="text-white font-medium text-lg">Sacred Act Complete</h3>
            <p className="text-purple-200">
              Your whisper has been {selectedAction === "lock-forever" ? "locked away forever" : "prepared for burning"}
            </p>
            <p className="text-gray-400 text-sm">
              Theme unlocked: "{selectedAction === "lock-forever" ? "Sacred Vault" : "Phoenix Ashes"}"
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">For secrets never meant to be shared</p>
          </div>
        </div>
        ...
        {lockedWhispers.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-white text-sm font-medium">Your Sealed Secrets</h4>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {lockedWhispers.filter(w => !w.burned).map((whisper) => {
                const ActionIcon = getActionIcon(whisper.action);
                return (
                  <div key={whisper.id} className={`rounded-lg p-3 border backdrop-blur-md ${getActionColor(whisper.action)}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <ActionIcon className="h-4 w-4" />
                        <span className="text-sm font-medium">
                          {whisper.action === "lock-forever" ? "Forever Locked" : "Ready to Burn"}
                        </span>
                      </div>
                    ...
                    <p className="text-xs opacity-70 mt-1">
                      Sealed {new Date(whisper.timestamp).toLocaleDateString()}
                    </p>
                  </div>
                );
              })}
        ...
        <div className="text-center p-4 bg-white/5 rounded-xl backdrop-blur-md">
          <p className="text-gray-300 text-sm leading-relaxed">
            Some thoughts are meant to be held, others released. This page is locked, but your feelings aren't.
          </p>
        </div>
      </div>
    </Card>
