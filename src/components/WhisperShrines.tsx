    ...
    <div className="flex flex-wrap justify-center gap-8">
      {shrines.map((shrine, idx) => (
        ...
        >
          ...
          <h3 className="kinetic-text text-xl font-bold whisper-gradient-text mb-2 text-center">{shrine.name}</h3>
          <p className="kinetic-text-slow text-base text-center text-gray-200 mb-2">{shrine.description}</p>
          <div className="mt-2 text-xs text-gray-400">Shrine grows as emotions gather here.</div>
        </div>
      ))}
    </div>
  );
