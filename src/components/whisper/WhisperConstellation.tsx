...
const WhisperConstellation: React.FC<WhisperConstellationProps> = ({
  whispers,
  onWhisperClick,
}) => {
  ...
  useEffect(() => {
    ...
    const trails = activeWhispers.map((whisper) => ({
      ...
      y: Math.random() * 100,
      targetX:
        zonePositions[whisper.zone as keyof typeof zonePositions]?.x || 50,
      targetY:
        zonePositions[whisper.zone as keyof typeof zonePositions]?.y || 50,
    }));
  ...
  return (
    <div className="relative w-full h-96 bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900 rounded-3xl overflow-hidden">
      ...
      {whispers.map((whisper) => {
        const zonePos =
          zonePositions[whisper.zone as keyof typeof zonePositions];
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        {whispers.slice(0, -1).map((whisper, index) => {
          ...
          if (
            whisper.zone === nextWhisper.zone &&
            whisper.mood === nextWhisper.mood
          ) {
            const pos1 =
              zonePositions[whisper.zone as keyof typeof zonePositions];
            const pos2 =
              zonePositions[nextWhisper.zone as keyof typeof zonePositions];
            if (pos1 && pos2) {
              return (
                <line
                  key={`line-${index}`}
