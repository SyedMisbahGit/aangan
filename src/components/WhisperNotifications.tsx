...
import { Bell, MessageCircle, Heart, Sparkles, AlertTriangle, Shield, X, CheckCircle, Clock, Users } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

interface Notification {
  id: string;
  type: "whisper" | "reaction" | "chain" | "milestone" | "moderation" | "system";
  title: string;
  message: string;
  timestamp: Date;
  isRead: boolean;
...
export const WhisperNotifications = () => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case "whisper": return MessageCircle;
      case "reaction": return Heart;
      case "chain": return Sparkles;
  ...
  if (!isExpanded) {
    return (
      <Card className="bg-white/5 backdrop-blur-lg border-white/10 p-4 hover:bg-white/10 transition-all duration-300 cursor-pointer" onClick={() => setIsExpanded(true)}>
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {notifications.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 text-xs">We'll alert you when something happens</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start space-x-3">
                    <Icon className={`h-5 w-5 mt-0.5 ${
                      notification.isRead ? "text-gray-500" : "text-purple-400"
                    }`} />
