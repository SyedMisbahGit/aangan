...
import { TrendingUp, Users, MessageCircle, Heart, BarChart3, Clock } from "lucide-react";
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-white/5 backdrop-blur-md border-white/10 p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-400">{analytics.dailyPosts}</div>
        ...
        <Card className="bg-white/5 backdrop-blur-md border-white/10 p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-400">{analytics.averageResponseTime}</div>
        ...
        <div className="space-y-3">
          {analytics.topCategories.map((category, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
              ...
              <Badge className={category.trend.startsWith('+') ? 'bg-green-500/20 text-green-200' : 'bg-red-500/20 text-red-200'}>
                {category.trend}
              </Badge>
            </div>
          ))}
      ...
      <Card className="bg-white/5 backdrop-blur-md border-white/10 p-6">
        ...
        <div className="space-y-4">
          <div className="flex h-4 bg-gray-700 rounded-full overflow-hidden">
            <div className="bg-green-500 h-full" style={{ width: `${analytics.sentimentBreakdown.positive}%` }} />
            <div className="bg-yellow-500 h-full" style={{ width: `${analytics.sentimentBreakdown.neutral}%` }} />
            <div className="bg-red-500 h-full" style={{ width: `${analytics.sentimentBreakdown.negative}%` }} />
          </div>

          <div className="flex justify-between text-sm">
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-green-500 rounded-full" />
              <span className="text-green-200">Positive ({analytics.sentimentBreakdown.positive}%)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-yellow-500 rounded-full" />
              <span className="text-yellow-200">Neutral ({analytics.sentimentBreakdown.neutral}%)</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-red-500 rounded-full" />
              <span className="text-red-200">Negative ({analytics.sentimentBreakdown.negative}%)</span>
            </div>
          </div>
        </div>
      </Card>
