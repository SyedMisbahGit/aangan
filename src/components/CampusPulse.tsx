...
import { MapPin, Users, Heart, Coffee, BookOpen, Music, Wifi, CloudRain, Sun, Moon, Zap, MessageCircle } from 'lucide-react';
...
const CampusPulse: React.FC = () => {
  ...
  const zones = [
    { id: '1', name: 'Central Library', type: 'library' as const, icon: BookOpen },
    { id: '2', name: 'Student Union Cafe', type: 'cafe' as const, icon: Coffee },
  ...
  const moods = {
    ...
    contemplative: { emoji: '🌙', color: 'text-blue-400', bg: 'bg-blue-400/20' },
    stressed: { emoji: '🌀', color: 'text-purple-400', bg: 'bg-purple-400/20' },
    joyful: { emoji: '💝', color: 'text-pink-400', bg: 'bg-pink-400/20' },
    nostalgic: { emoji: '💫', color: 'text-indigo-400', bg: 'bg-indigo-400/20' },
  };
  ...
  useEffect(() => {
    ...
    const updatePulseData = () => {
      ...
      const updatedZones = zones.map(zone => ({
        ...
        mood: Object.keys(moods)[Math.floor(Math.random() * Object.keys(moods).length)],
      ...
      setPulseData({
        ...
        currentMood: Object.keys(moods)[Math.floor(Math.random() * Object.keys(moods).length)],
        weather: ['clear', 'cloudy', 'rainy', 'sunny'][Math.floor(Math.random() * 4)],
        timeOfDay,
        zones: updatedZones,
      });
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="glass border-accent/20">
          <CardContent className="p-4 text-center">
            <div className="flex items-center justify-center gap-2 mb-2">
              <span className="text-2xl">{moods[pulseData.currentMood as keyof typeof moods]?.emoji}</span>
              <span className="text-lg font-bold text-accent">{pulseData.currentMood}</span>
            </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-muted-foreground capitalize">{zone.mood}</span>
                      </div>
