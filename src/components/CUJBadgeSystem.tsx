...
import { GraduationCap, Heart, Home, TestTube, Music, BookOpen, Users, Coffee, Star, Zap, Moon, Sun } from 'lucide-react';
...
const CUJBadgeSystem: React.FC = () => {
  ...
  const categories = {
    'all': { label: 'All Badges', icon: Star, color: 'bg-primary/20 text-primary' },
    'hostel': { label: 'Hostel Life', icon: Home, color: 'bg-pink-500/20 text-pink-500' },
    'academic': { label: 'Academic', icon: BookOpen, color: 'bg-blue-500/20 text-blue-500' },
    'cultural': { label: 'Cultural', icon: Music, color: 'bg-yellow-500/20 text-yellow-500' },
    'personality': { label: 'Personality', icon: Heart, color: 'bg-green-500/20 text-green-500' },
    'achievement': { label: 'Achievements', icon: GraduationCap, color: 'bg-purple-500/20 text-purple-500' },
  };
  ...
  const totalProgress = cujBadges.reduce((sum, badge) => sum + (badge.progress / badge.maxProgress), 0) / cujBadges.length * 100;
      ...
      <Card className="glass border-primary/20">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-secondary">{cujBadges.length}</div>
                    ...
                    <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full transition-all duration-300 ${
                          badge.unlocked ? 'bg-primary' : 'bg-muted-foreground/30'
                        }`}
          ...
          <CardContent className="space-y-4">
            <div className="p-4 rounded-lg bg-primary/5 border border-primary/10">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                <strong>Cultural Context:</strong> {selectedBadge.culturalContext}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-3 bg-muted rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-300 ${
                      selectedBadge.unlocked ? 'bg-primary' : 'bg-muted-foreground/30'
                    }`}
